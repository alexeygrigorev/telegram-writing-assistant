...
- **Determinism.** During replay, the workflow code must follow the same path it followed the first time, so the recorded results line up with the code that asks for them. That's why engines make you separate workflow logic from side effects: the workflow decides, the *activities* (or steps) do the doing, and only the doing gets journaled. I found out how literally engines enforce this while building a Temporal workflow of my own: the moment my workflow code imported an Elasticsearch client capable of making its own HTTP calls, Temporal's sandbox refused to even start the workflow, and only let it through once I wrapped the import in workflow.unsafe.imports_passed_through() to declare the side effect intentional.
...
- [Conductor](https://github.com/conductor-oss/conductor), stewarded by Orkes and open source, takes a fourth shape: a general-purpose workflow engine, originally built at Netflix in 2016 to orchestrate microservices without touching their code, that now compiles agent definitions directly into workflows of its own.

[Nicholas Lotz's DataTalks.Club workshop on running durable agents in production](https://www.youtube.com/live/KdmX-ZPuKbs) is a good look at what that compilation buys you underneath. Conductor runs as a client-server system: the server holds durable execution state (Postgres in production, SQLite for a single-container demo), and workers are disposable processes that pull tasks off a queue, execute them, and report the result back. Kill the worker mid tool-call in his demo and the workflow doesn't error, it just sits at RUNNING and waits for a replacement worker to pick the task back up, the suspension property from earlier made concrete. Marking a tool approval_required=True compiles it into what Conductor calls a human task: the workflow pauses indefinitely and only continues once something calls runtime.approve() with the approval id. And because every SDK, Python, Java, JavaScript, C#, Go, Rust, compiles down to the same JSON workflow representation, Conductor wraps agents built in LangGraph, the OpenAI Agents SDK, or Google's ADK just as readily as its own native agent definitions.
...
It's also worth separating durable execution from workflow orchestration in the Airflow or Dagster sense, since the word workflow gets recruited for both. When I ran a live workshop turning a podcast-transcript ingestion pipeline into a Temporal workflow, this was the recurring question: why not just use the orchestrator already running in the stack. The honest answer is that batch orchestrators are built for a mostly-fixed DAG of long-running steps, typically hours apart, pulling from a warehouse on a schedule. An agent loop is the opposite shape: the step count isn't known until you're inside it, waits on external events can run from milliseconds to months, and a crash mid-step has to resume without losing the conversation, not just rerun a batch job from its last checkpoint. That's a different tool for a different failure mode, which is why teams adopting Temporal or Conductor for agents are reaching past their existing Airflow setup rather than extending it.
...
A smaller-scale data point from the same rung, and my own experience rather than a vendor's: building a deep-research agent over a podcast archive, I wrapped a Pydantic AI agent in pydantic_ai.durable_exec.temporal.TemporalAgent and every one of its tools was promoted to a Temporal activity automatically, no hand-written activities module required. The one snag was a tool that took a RunContext argument, which Temporal's activities need to be serializable and Pydantic AI's default RunContext isn't, so the fix was a small custom serializer, not a redesign. The same workshop had already shown the payoff on the ingestion side of the pipeline: a live SSL error against a flaky residential proxy got retried automatically, no lost work and no restart from scratch.
