...
<figure>
  ...
  <!-- Concrete example of the playlist automation described in this paragraph, before it has been turned into a skill - the context is supplied as docs rather than a skill -->
</figure>
...
The homework automation I described above is a real example of all this, and it shows the other end of the range too - a project that is nothing but skills. It lives in a separate repository, [course-management-agent](https://github.com/alexeygrigorev/course-management-agent), and that project really is just skills and nothing else: there is no application code in it, only a .claude folder with skill files. It has three skills, each a single SKILL.md that drives the platform's REST API with curl - one for getting and creating homeworks and projects, one for creating and updating the questions inside a homework, and one for triggering a production deployment[^39][^40]. The [homework-questions skill](https://github.com/alexeygrigorev/course-management-agent/blob/master/.claude/skills/homework-questions/SKILL.md), for example, is pure instructions: look up the homework's numeric ID, read the local homework file and the solutions file to get the correct answers, check how many questions already exist (the API appends rather than replaces), post the questions, then open the homework[^40]. So a skill can hold scripts and templates, but it does not have to - these are just instructions. The platform they upload to is our [course management platform](https://github.com/DataTalksClub/course-management-platform), a Django app hosted at [courses.datatalks.club](https://courses.datatalks.club/) that runs the DataTalks.Club Zoomcamps[^41].
