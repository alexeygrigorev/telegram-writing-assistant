...
| Title | Status | Last Updated | Description |
|-------|--------|--------------|-------------|
| [Sandbox AWS Access Without Distributing Keys](sandbox-aws-access.md) | draft | 2026-06-10 | Talk on giving AWS access to workshop attendees and to coding agents without distributing long-lived keys: how instance profiles work, replicating them outside AWS with AWS_CONTAINER_CREDENTIALS_FULL_URI and a Lambda, a sandbox account + dev-container workshop setup, an SSH sandbox server for agents, a CLI that drops temporary credentials into an agent's project folder, and a phone toggle that gates sandbox access |
...
| [Approaching Design with AI as a Non-Designer](designing-with-ai.md) | draft | 2026-06-09 | Non-designer's mockup-first workflow for getting good-looking UIs out of AI: recognizing the characteristic AI design tics (buttons on the right, over-dense columns, icon-title-blurb feature grids), then GPT Image 2 / ChatGPT generates mockups to set the direction, a code agent (Codex) implements them, then you pick an existing design system (GitHub) as a reference, write a design-guidelines doc, and polish details in code. Started on the DataTalks.Club course platform Tailwind migration, then reused on the Pocket Shell Android app with a screenshot-based iteration loop |
...
| [Substack Archive Index](substack-archive-index.md) | draft | 2026-06-06 | Lookup table of all Alexey's published Substack posts with title, description, and URL. Latest entries include coding-agent setup and minsearch. Used to insert direct links when he references something he has already shared |
