...
| Title | Status | Last Updated | Description |
|-------|--------|--------------|-------------|
| [Personalised Plans](ai-shipping-labs/plans/_index.md) | draft | 2026-05-03 | Personalised plans with Alexey's recommendations: Luca (Italian researcher, ~22 yrs in research/education institutes, Hermes coding agent, GAIL cert prep; ship Chat with My Notes RAG over Obsidian in 6 weeks with the agent loop written by hand, AI Hero in parallel, weekly LinkedIn series, mirror Field Guide language for research-institute AI Engineer framing), Koray Can Canut (2 hr/week, agent-fallback guidance), Sai Kumar G (Telegram News Event Reminder bot, build agent yourself, sync with Buildcamp arc), Diogo Valente Polónia (single-agent expert elicitation, deploy as part of building), Dianne Bronola (Learning Companion Agent capstone, design data structure first, run workflow manually before automating). Plus all personal plan files renamed with `YYYYMMDD_` creation-date prefix for easier directory sorting (only applies to plan files in this folder). Existing plans for Vishnu, Motasem Salem, Kushal Kulshreshtha, Edu Gonzalo Almorox, Grace, Vancesca Dinh, Carlos Pumar, Jakob Zischka, Nirajan Acharya unchanged |
