...
| Title | Status | Last Updated | Description |
|-------|--------|--------------|-------------|
| [Personalised Plans](ai-shipping-labs/plans/_index.md) | draft | 2026-05-02 | Personalised plans with Alexey's recommendations: Koray Can Canut (corrections - 2 hr/week not 4, removed pre-sprint framing, added agent-fallback guidance), Sai Kumar G (Telegram News Event Reminder bot, build agent yourself rather than hand to Claude Code, sync with Buildcamp's tools/tests/monitoring/evolution arc), Diogo Valente Polónia (third project - expert elicitation - reduced to a single agent, deploy as part of building), Dianne Bronola (Learning Companion Agent capstone, design data structure first, run workflow manually before automating). Plus all personal plan files renamed with `YYYYMMDD_` creation-date prefix for easier directory sorting (only applies to plan files in this folder). Existing plans for Vishnu, Motasem Salem, Kushal Kulshreshtha, Edu Gonzalo Almorox, Grace, Vancesca Dinh, Carlos Pumar, Jakob Zischka, Nirajan Acharya unchanged |
