...
|-------|--------|--------------|-------------|
| [Minsearch: Simple Search for Small Datasets](ready-for-newsletter/minsearch-library.md) | draft | 2026-05-28 | Lightweight search library for small to medium datasets: where it fits and where it's used, the origin story, TF-IDF design, and its evolution from a single file to a PyPI package |
...
| Title | Status | Last Updated | Description |
|-------|--------|--------------|-------------|
| [Setting Up Your Coding Agents](setting-up-coding-agents.md) | draft | 2026-06-05 | Story-arc guide to setting up Claude Code, Codex, and OpenCode around a zero-to-hero path: start simple and discover your own workflow, choose an assistant, solve tasks directly, permissions and YOLO mode, automation, make skills (what to put in a skill, a skill is a folder not just a markdown file, the Anthropic skills article and OpenAI's /home/oai hidden skills), share skills across tools via symlinks, use subagents, context files (CLAUDE.md/AGENTS.md), the reset and goal subcommands, his public .claude dotfiles repo, with docs/courses and per-tool notes as reference at the end |
...
| [Redesigning the DataTalks.Club Course Platform with ChatGPT Mockups](dtc-course-platform-redesign.md) | draft | 2026-05-19 | Mockup-first redesign workflow on the DataTalks.Club course management platform: describe the site to ChatGPT page by page (web + mobile), iterate on the generated images until the style is right, hand them to a code agent (Codex on Tailwind) to reproduce the layout, then polish the small details in code with a style-guidelines doc because pointwise edits do not work through the screenshot generator |
| [Stylint: Enforcing My Writing Style on AI Assistants](stylint.md) | draft | 2026-05-19 | New Python linter library at github.com/alexeygrigorev/stylint that codifies writing-style rules as code instead of long markdown style guides. Two components: deterministic regex/pattern rules (banned words, banned phrases, long sentences, abstract noun framing, lazy headings) and minimal markdown reference docs for judgment-level decisions. Motivation: style files grew too big, agents started skipping rules, and Claude Code / Codex / OpenAI Codex CLI all needed a shared enforcement layer |
| [Working from a Phone](working-from-phone.md) | draft | 2026-05-16 | Mobile workflow on a dedicated remote server: Termius + tmuxctl, three agents with short aliases (csp, cy, oc), AWS sandbox isolation for runaway agents (with the dropped-production-DB incident as motivation), dotfiles, Typeless dictation, Google Recorder background audio handed to an orchestrator agent that turns it into GitHub issues, ssh-auto-forward-android, agent-uploaded screenshots and throwaway HTML pickers (banner review), Telegram writing assistant for in-transit thoughts, behind-the-scenes + time accounting for the brain-dump + edit workflow |
