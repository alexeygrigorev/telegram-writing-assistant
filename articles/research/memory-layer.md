...
created: 2026-05-15
updated: 2026-08-09
tags: [research, memory, agents, llm, production, chatgpt, claude-code, mem0, letta, graphiti, cognee, langmem, a-mem, retrieval]
status: draft
---

# Memory Layers for AI Agents

This article is a tour of how memory is built inside production AI systems. The trigger is a concrete task: a startup asked me to design the memory layer for an AI agent, and I wanted to see how teams that already shipped this in production solved it.[^1][^2][^15]

The production implementations are the core of this article, followed by complementary patterns: BM25, GitHub Copilot's citation verification, fixed ontologies, SQL-versus-vector trade-offs, file-based memory, and MemPalace's hierarchical local design.
...
Pain points raised in the [storage-engine debate](#sql-vectors-and-graphs-match-storage-to-memory-type):
...
The extraction LLM has a docstring-style prompt that tells it to extract "atomic, self-contained facts about the user". If the conversation does not contain anything worth extracting, return an empty list. This is the same pattern the [custom-memory implementation](#custom-memory-as-extract-embed-retrieve-and-maintain) describes.
...
This solves the staleness problem differently than [GitHub's citation-based verification](#github-copilot-verify-citations-at-read-time). Instead of verifying at read time, Graphiti tracks validity windows at write time and invalidates contradictions automatically.
...
This connects to the [Fixed Entity Architecture](#fixed-entity-architecture-curate-the-ontology) argument - manually defined domain concepts plus learned connections.
...
- Graph plus vectors in one pipeline avoids the multi-system complexity raised in the [storage-engine debate](#sql-vectors-and-graphs-match-storage-to-memory-type)
...
## Complementary Memory Patterns

The systems above are not the whole design space. Earlier research and practitioner discussions add several useful patterns and cautions.

### BM25 as a Lightweight Baseline

Eric Tramel's now-unavailable article described searchable agent memory using BM25 over plain text.[^18] The attraction is operational simplicity: no embedding computation or vector service, interpretable term-based scores, and surprisingly good retrieval when the vocabulary in memories and queries overlaps. BM25 is a useful baseline before semantic retrieval is justified.

### Custom Memory as Extract, Embed, Retrieve, and Maintain

A practical DSPy and Qdrant implementation inspired by Mem0 separates memory into four optional operations: extract atomic facts, embed them, retrieve them when context requires it, and maintain them with add, update, delete, or no-op decisions.[^19]

The implementation stores a short self-contained fact with `user_id`, date, text, and embedding. A DSPy signature defines extraction as a structured contract; a ReAct response agent decides whether it needs the `fetch_similar_memories` tool. Qdrant's payload filter enforces user isolation alongside similarity search. The most transferable lesson is that retrieval and saving should both be conditional: irrelevant memory consumes tokens and can lower answer quality.

### GitHub Copilot: Verify Citations at Read Time

GitHub Copilot shares repository-scoped memories among coding agent, CLI, and code review. Each memory contains a subject, fact, reason, and citations to supporting code locations.[^20] At retrieval time, the agent checks those citations. Valid evidence refreshes the memory; contradictory code causes the agent to correct it.

This turns staleness from an expensive continuous-curation problem into a small verification task at use time. Only users with write access create repository memories, while users with read access can consume them. GitHub reported improved code-review precision and recall in its A/B test, and the shared store also transfers knowledge discovered by one agent to the others.

### Fixed Entity Architecture: Curate the Ontology

A code-migration case study argues against asking an LLM to discover a knowledge-graph ontology from scratch.[^21] Its Fixed Entity Architecture has three layers: domain-expert-defined concepts, embedded document chunks, and entities extracted deterministically with NLP or regular expressions.

Rich descriptions and hypothetical code examples bridge the embedding-space gap between natural-language concepts and code. Cosine thresholds connect chunks to concepts, while reciprocal-rank fusion combines vector, full-text, and concept-guided results. Excluding over-broad "super-node" concepts is as important as selecting useful ones. This approach is appropriate when the domain is known and reproducibility matters more than open-ended entity discovery.

### SQL, Vectors, and Graphs: Match Storage to Memory Type

A community discussion around the Memori SQL-native engine produced a more useful conclusion than "SQL wins": different memory types need different retrieval semantics.[^22]

- SQL works well for explicit preferences, rules, entities, dates, and relationships known in advance.
- Vectors support fuzzy semantic recall when the query and memory use different language.
- Graphs support multi-hop relationship queries.
- Postgres with pgvector is a pragmatic starting point when one system should handle structured facts and semantic retrieval.

The complexity should follow demonstrated needs. Start with typed records and indexes; add semantic or graph retrieval only for queries the simpler model cannot answer.

### File-Based Memory for Coding Agents

A debate about Claude Code memory makes the case for repository documentation as a legitimate memory layer: `CLAUDE.md`, `MEMORY.md`, skills, READMEs, plans, task files, and commit history are version-controlled, human-readable, and naturally scoped.[^23][^24]

The limitations are scale and triggering. Large multi-repository codebases can require semantic search, and agents do not always decide to save or load memory reliably without reminders. An effective pattern is a small index file linking to topic files, loaded lazily with search. Every injected memory still has to justify its context-window cost.

### MemPalace: Hierarchy and Progressive Loading

MemPalace is a fully local system organized as wings, rooms, halls, and drawers.[^25] It combines ChromaDB, a SQLite temporal knowledge graph, regex-based extraction, and progressive loading so startup context remains small. Its published LongMemEval result is 96.6% recall@5, with the hierarchy reported to improve retrieval over flat vector search. The design reinforces a theme visible in Claude Code and A-MEM: navigable organization can matter as much as the embedding model.

### Session Persistence Is Adjacent, Not Semantic Memory

The Vibe Company's Companion persists separate Claude Code sessions to disk and resumes their processes after crashes through a reverse-engineered WebSocket interface.[^16][^17] This preserves conversation continuity and isolates parallel sessions, but it does not decide what facts matter or retrieve knowledge across them. It belongs at the session layer beneath a semantic memory system.
...
- Postgres with pgvector for combined structured queries and embeddings, matching the [pragmatic middle ground](#sql-vectors-and-graphs-match-storage-to-memory-type)
...
[^15]: Initial research notes: [20260209_221323_AlexeyDTC_msg1259.md](../../inbox/used/20260209_221323_AlexeyDTC_msg1259.md), [20260209_221536_AlexeyDTC_msg1260.md](../../inbox/used/20260209_221536_AlexeyDTC_msg1260.md), and [20260210_075929_AlexeyDTC_msg1261_transcript.txt](../../inbox/used/20260210_075929_AlexeyDTC_msg1261_transcript.txt)
[^16]: [The-Vibe-Company/companion](https://github.com/The-Vibe-Company/companion)
[^17]: [20260210_082618_AlexeyDTC_msg1262_photo.md](../../inbox/used/20260210_082618_AlexeyDTC_msg1262_photo.md)
[^18]: [Eric Tramel, "Searchable Agent Memory"](https://eric-tramel.github.io/llm/2024/11/14/memory.html) (original URL now returns 404)
[^19]: ["How to Build Your Own Custom LLM Memory Layer from Scratch"](https://towardsdatascience.com/how-to-build-your-own-custom-llm-memory-layer-from-scratch/) and its [mem0-dspy implementation](https://github.com/avbiswas/mem0-dspy)
[^20]: [GitHub, "Building an agentic memory system for GitHub Copilot"](https://github.blog/ai-and-ml/github-copilot/building-an-agentic-memory-system-for-github-copilot/)
[^21]: ["Why I Stopped Letting LLMs Build My Knowledge Graphs"](https://www.linkedin.com/pulse/why-i-stopped-letting-llms-build-my-knowledge-graphs-what-malaraju-ycxbc), introduced in [20260211_153046_AlexeyDTC_msg1452.md](../../inbox/used/20260211_153046_AlexeyDTC_msg1452.md)
[^22]: [Reddit discussion: "Everyone's Trying Vectors and Graphs for AI Memory. We Went Back to SQL."](https://www.reddit.com/r/AI_Agents/comments/1nkx0bz/everyones_trying_vectors_and_graphs_for_ai_memory/), introduced in [20260211_152925_AlexeyDTC_msg1451.md](../../inbox/used/20260211_152925_AlexeyDTC_msg1451.md)
[^23]: [Reddit discussion: "Please Stop Creating 'Memory for Your Agent' Frameworks"](https://www.reddit.com/r/ClaudeCode/comments/1r4asf6/please_stop_creating_memory_for_your_agent/)
[^24]: [20260214_063525_AlexeyDTC_msg1657.md](../../inbox/used/20260214_063525_AlexeyDTC_msg1657.md)
[^25]: [MemPalace repository](https://github.com/milla-jovovich/mempalace/tree/main), introduced in [20260407_120000_AlexeyDTC_msg3256.md](../../inbox/used/20260407_120000_AlexeyDTC_msg3256.md)
