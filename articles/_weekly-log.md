...
Gave a talk about authentication this week - how to give AWS access to people and to coding agents without distributing long-lived keys. Based on this talk I'll write an article next week. See [The System I Built for AWS Access Without Keys](https://aishippingblog.com/p/the-system-i-built-for-aws-access)[^50][^51].

## Week of 2026-05-25

## pocketshell and a Team of Agent Teams

pocketshell, the Android app that packages my phone-based development workflow, reached a state where I can actually use it this week while continuing to build it[^47]. It is the next step from the mobile workflow described in [Working from a Phone](https://aishippingblog.com/p/the-system-i-built-to-ship-code-from): last week I mentioned that some parts of that process were still being set up, and pocketshell is about packaging everything into one box where I can do everything I need to do[^48].

Ran an experiment on it: coordinating three coding agents - Codex, Claude Code, and OpenCode - as three team managers that find each other, divide up the work, and collaborate on the same repo. See [A Team of Agent Teams](https://aishippingblog.com/p/minsearch-the-small-search-library) for the full story[^47].
...
## Slack Moderation Agent Update

Updated our Slack agent. Spammers had really worn me out, and I was spending a lot of time cleaning them up. Deleting a spammer was a lot of manual work: first I had to go through all the channels - and I am not present in some channels, so I could miss the spam - then delete the spam, then go to the admin panel, open the members tab, and enter the user's name (I cannot see the user ID from the phone). If several users had the same name, I also had to compare avatars to pick the right one. When the avatars are auto-generated by Slack, it is hard to tell who I am about to delete, and there is a chance I accidentally ban the wrong user[^43].

I already had a moderation system for this but never finished it. This time I decided to make it simpler, and I finally understood how. The problem is that Slack has no way to ban a user through the API. So I added a new function to Automator (the Slack moderation bot I have [written about before](https://aishippingblog.com/p/building-and-maintaining-a-slack)). Now I add a reaction to a message, and the bot finds that user's messages from the last 24 hours and deletes them from all channels - even channels I am not a member of. After all the messages are deleted, the bot sends me a message with the user's ID, which I can copy, paste into the admin panel, and use to unambiguously identify and ban the person[^43][^44].

<figure>
  <img src="../assets/images/weekly-log/slack-moderation-ban-cleanup.jpg" alt="Au-Tomator notification: Ban cleanup completed, listing the user ID and the manual deactivation steps in the admin panel">
  <figcaption>Au-Tomator's "Ban cleanup completed" message - it deletes the spammer's messages, then hands back the exact user ID for the manual deactivation step</figcaption>
  <!-- Illustrates the new reaction-driven flow: the bot cleans up messages and returns the user ID so the admin-panel ban is unambiguous -->
</figure>

I could not fully automate it, but the reaction time is now much faster. The old problem is gone too: if a user had a name like "A", it used to be very hard to find and ban the right person because roughly half of Slack matched. Now I just copy the ID, paste it into the admin panel, and the person is banned[^43].

## O'Reilly Guardrails Workshop

Worked on a workshop I am running next week, together with O'Reilly on their O'Reilly Learning platform, about guardrails: [Building an Agent with Guardrails](https://aishippinglabs.com/workshops/2026-05-26-agent-with-guardrails)[^45][^46]. The workshop builds a FAQ agent and layers guardrails around it - input guardrails that check topic and intent, output guardrails that catch unsafe promises and policy violations, and concurrent multi-guardrail checks to keep latency low[^46].

I am collecting all the workshops I have run at conferences and elsewhere in [one place](https://aishippingblog.com/p/11-workshops-to-build-production), and this one is no exception. It is still a draft version but already published on the site, and I will keep updating it as things change[^45].
...
The agent workflow works but requires more babysitting - no task widget, no auto-continue when subagents finish. Limits on Codex feel much more generous. See [Trying OpenAI Codex as a Claude Code Alternative](https://aishippingblog.com/p/5-useful-utilities-i-built-with-ai) for details[^22][^23][^24].
...
Spent time converting a ChatGPT-generated PNG logo into SVG using Claude Code with OpenCV. Went through multiple approaches before finding one that works. See [Recreating a PNG Logo as SVG with Claude Code](https://aishippingblog.com/p/how-we-built-ai-shipping-labs) for the full story[^3].
...
Created a logo with ChatGPT and attempted SVG recreation. All integrations now run end to end across Gmail and GitHub and Zoom and Slack and Stripe. See [How We Built AI Shipping Labs Website using AI Tools](https://aishippingblog.com/p/how-we-built-ai-shipping-labs) for details[^4][^5][^6].
...
[^43]: [20260521_105031_AlexeyDTC_msg4270_transcript.txt](../inbox/used/20260521_105031_AlexeyDTC_msg4270_transcript.txt)
[^44]: [20260521_104650_AlexeyDTC_msg4268_photo.md](../inbox/used/20260521_104650_AlexeyDTC_msg4268_photo.md)
