...
The real processing instructions, subagent definitions, and related prompts live in the `process/` folder at the repo root:

- `process/process.md` - this file, the full inbox processing workflow
- `process/researcher.md` - body for the `researcher` subagent
- `process/article-summarizer.md` - body for the `article-summarizer` subagent
- `process/resource-describer.md` - body for the `resource-describer` subagent
- `process/verify-content.md` - body for the `verify-content` subagent

The files in `.claude/commands/` and `.claude/agents/` are thin stubs that just point back to these files. Do not put instructions in the stubs.

If the user asks you to update, fix, or extend any of these instructions (for example: "update the researcher agent to do X", "add a rule to process.md about Y", "fix the verify-content agent"), edit the files in `process/`, not the ones in `.claude/`. Treat `.claude/` as read-only for instruction content - it only exists so Claude Code can discover the agents and commands.
...
2. Identify the target file: Which file should be updated?
   - `process/process.md` - for workflow, categorization, and processing rules
   - `process/{agent-name}.md` - for subagent prompt/instruction updates (e.g., `process/researcher.md`)
   - An article file - for content corrections
   - `_index.md` - for index-related changes
   - Never edit files in `.claude/` for instruction updates - those are stubs
...
5. Update the target file: Make the change to prevent recurrence. CRITICAL: If the user asks to update `process/process.md`, you MUST actually edit it in this session. Do not defer it or skip it.
